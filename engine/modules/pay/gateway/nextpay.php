...
class Nextpay_Payment
{
    ...
    public function token(){
        $res = (object)array("trans_id"=>"", "code"=> -1000);
        $return = $this->token_request();
        return $return;
        for ($i = 0; $i <= 2; $i++) {
            $this->default_verify = $i;
            $return = $this->token_request();
            if($return && $return != "" && $return != NULL && is_object($return)) {$res = $return; break;}
        }
        return $res;        
    }
    ...
    public function verify($params=false, $api_key=false, $order_id=false, $trans_id=false, $amount=false){
        ...
        for ($i = 0; $i <= 2; $i++) {
            $this->default_verify = $i;
            $res = $this->verify_request($params, $api_key, $order_id, $trans_id, $amount);
            if($res != -1000)break;
        }
        return $res;
    }
    ...
    public function token_request()
    {
        ...
        switch ($this->default_verify)
        {
            ...
            case Type_Verify::NuSoap:
                try
                {
                    $nusoap_file = 'nusoap.php';

                    if (file_exists($nusoap_file)) include_once ("nusoap.php");
                    else break;
    ...
    public function link_payment($trans_id){
        if(isset($trans_id)) return $this->request_http."/".$trans_id;
        else return $this->request_http."/".$this->trans_id;
    }
...
function Request($apikey, $amount, $callback_url){
	$order_id = rand();
	$params = array(
        "api_key"=>$apikey,
        "amount"=>$amount,
    ...
    $result = $nextpay->token_request();
    if(intval($result->code) == -1){
        //$nextpay->send($result->trans_id); //test
        return array(
			'status' => true,
			'ref'    => $result->trans_id,
			'url'    => $nextpay->link_payment($result->trans_id)
		);
    }
    else
    {
        return array(
			'status' => false,
			'msg'	 => $nextpay->code_error($result->code)
		);
    }
		
}

function Verify($apikey, $trans_id, $order_id, $amount){
	...
	if ($result == 0) {
		return array(
			...
			'statuscode' => $result,
			'ref'    => $trans_id,
			'msg'    => $nextpay->code_error(0)
		);
	}else
		return array(
			'status' => false,
			'statuscode' => $result,
			'msg'    => $nextpay->code_error(-2)
		);
}
